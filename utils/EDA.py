''' This module keeps all the basic functions for the exploratory data analysis 
...
def hist_box(df:pd.DataFrame, feature:str, avoid_zeros:bool=False)->None:
    ...
    sns.boxplot(data = df2, x = feature, ax = ax_box, showmeans = True)#, color = "pink")        
...
def heatmap_diag(df:pd.DataFrame, annot:bool=False, fontsize:int=16)->None:
    ...
    sns.heatmap(data = corr, mask = mask, vmin = -1, vmax = 1, annot = annot, fmt = ".2f", cmap = 'RdBu', annot_kws = {"size": fontsize}) #cmap = "Spectral");
    plt.show();
...
def pairplot_reg(df:pd.DataFrame, features:list, only_day:bool=False)->None:
    ...
    g = sns.pairplot(
        ...
        plot_kws={'line_kws':{'color':'red'}, 'scatter_kws':{'s': 1, 'alpha': 0.1}}, 
        corner=True);
...
def boxplots(df:pd.DataFrame, x:str='Month', y:list=['GHI', 'DNI', 'DHI'])->None:
    ...
    '''
    fig, axs = plt.subplots(nrows=len(y), ncols=1, figsize=(16, 16), linewidth=0.5, alpha=0.4)
...
def hourly_irrad(df:pd.DataFrame, y:list=['GHI', 'DNI', 'DHI'])->None:
    ...
    plt.title('Hourly Solar Irradiance consumption in $KW-hr/m^2/day$ averaged over 21 years (2001-2021)')
    plt.show();
...
def daily_irrad(df:pd.DataFrame, y:list=['GHI', 'DNI', 'DHI'])->None:
    ...
    plt.title('Daily Solar Irradiance consumption in $KW-hr/m^2/day$ averaged over 12 months (Jan-Dez)')
    plt.show();
...
def monthly_irrad(df:pd.DataFrame, y:list=['GHI', 'DNI', 'DHI'])->None:
    ...
    plt.title('Monthly Solar Irradiance consumption in $KW-hr/m^2/day$ averaged over 21 years (2001-2021)')
    plt.show();
...
def trend(df:pd.DataFrame, feature:str, year:str)->None:
    ...
    '''
    if year=='all':
        detect_trend(df=df[feature].resample('M').sum().values, text='2001 - 2021', feature=feature)
    else:
        detect_trend(df=df[feature].resample('M').sum().loc[year].values, text=year, feature=feature)
