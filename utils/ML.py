...
def data_check(df: pd.DataFrame) -> None:
    ...
    It contains data type of each feature, number of "nan" values, duplicated rows,
    data description with statistical information and so on.
...
def plot_predvstrue_reg(pred, truth, model_name=None):
    ...
    _ = plt.xlabel(
        f'Observed Solar Irradiance ({truth.name}) in $KW-hr/m^2/day$')
    _ = plt.ylabel(
        f'Predicted Solar Irradiance ({truth.name}) in $KW-hr/m^2/day$')
    _ = plt.title(
        f'Observed vs Predicted Solar Irradiance ({truth.name}) using {model_name}')
    # plt.xlim(-100, 1000)
    # plt.ylim(-200, 1300)
...
def residuals_plot(X, y, model: linear_model, hist=False):
    ...
    :param model: _description_
    :param hist: _description_, defaults to False
    """
    ...
    if model_text == 'LinearRegression':
        ...
        _ = plt.title(
            f'Fitted values versus Residuals for {model_text} Regression')
    if hist:
...
def train_test(
    ...
    if plot:
        _ = plt.figure(figsize=(16, 6))
        _ = plt.title(
            f'Solar Irradiance ({target}) train and test sets', size=20)
    ...
    if scale and not include_test_scale:
        scaler = StandardScaler()
        X_train[cols_to_transform] = scaler.fit_transform(
            X_train[cols_to_transform])
...
def trend_model(data, target, cols_to_transform, l1_space, alpha_space, cols_use=0, scale=True, test_size=0.15,
    ...
    _ = plt.xticks(range(len(X_train.columns)),
                   X_train.columns.values, rotation=90)
    _ = plt.margins(0.02)
    _ = plt.axhline(0, linewidth=0.5, color='r')
